...
module.exports = {
    ...
    getFeaturedProducts: () => {
        return new Promise(async (resolve, reject) => {
            let featuredProds = await db.get().collection(collection.PRODUCT_COLLECTION).find({ Category: 'Featured Products' }).toArray()
            resolve(featuredProds)
        })
    },
    getSpecialProducts: () => {
        return new Promise(async (resolve, reject) => {
            let specialProds = await db.get().collection(collection.PRODUCT_COLLECTION).find({ Category: 'Special Products' }).toArray()
            resolve(specialProds)
        })
    },
    getTrendingProducts: () => {
        return new Promise(async (resolve, reject) => {
            let trendingProds = await db.get().collection(collection.PRODUCT_COLLECTION).find({ Category: 'Trending Products' }).toArray()
